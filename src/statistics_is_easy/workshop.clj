...
;
; Create a large number of buckets to draw from  and split them up based on the p parameter. Any random drawing that draws from the left side is considered a success
;
...
;
;A **binomial distribution** with parameters n and p is the discrete probability distribution of the number of p successes in a sequence of n independent experiments
;
...
;#### Other examples
;Take the instance of an online game company that introduces a new feature where the normal retention rate is 70% after introducing the feature we observe that 38 of 97 people revisit the site. Is there a real problem with this feature?
;
;When doing an A/B experiment the normal click through rate(Number of users clicking impressions)  is 2%. The adtech company has changed the way ads are being selected for display and observes that of the 900 impressions presented 30 users clicked into the ads. Is the new selection system better
...
;
;Lets run an experiment where many many times we shuffle the labels and check the difference in the average for each shuffled drug. Each time we see a value greater than the observed we increment a counter. 
;
